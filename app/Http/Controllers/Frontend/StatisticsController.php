...
class StatisticsController extends Controller
{
    ...
    public function getPerClient(){
        ...
        $result = DB::select('SELECT name,MAX(total) as total_amount FROM `quotes`  where status = "paid"  group by (name) order by  total asc LIMIT 5');
    ...
    public function totalUnconvertedQuotes(){
        ...
        $total_non_converted_quotes = Quote::where('status','unpaid')->where('is_send',0)->count();
    ...
    public function totalUnpaidBill(){
        ...
        $total_unpaid_bills = Quote::where('status','unpaid')->where('is_send',1)->count();
