    ...
    <div class="profile_box">
      <div class="menu_box_table">
          ...
          <div class="profile_box_text">
            ...
            @if(count($workAddressArr) > 0)
              ...
              @foreach($workAddressArr as $key=>$address)
                <p >{{ $address['street'] }} {{ $address['street_number'] }} ; {{ $address['postal_code'] }} {{ $address['municipality'] }} <a class="newquote_link" href="{{route('quote.single', ['clientId'=> Hashids::encode($client->id),'id'=> $address['id']])}}">{{$showLinkText}}</a></p>
        ...
        <div class="profile_box_tableCell" style="width: 50px !important;">
          <div class="client_list_arrow" style="display: {{count($workAddressArr) == 0 ? 'none' : 'block'}}">
