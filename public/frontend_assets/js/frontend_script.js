 ...
 $("body").on('click','.add_client_pg_btn', function () {
  ...
  // 3rd phase
    ...
    $("body").on('click', '.delete_quote_button', function () {
        ...
        $(".delete_confirm_popup_box").show();
        let url = app_url+'/quote/delete/'+id;
        $(".confirm_del_btn").unbind('click');
        $(".confirm_del_btn").click(function () {
            getAjaxData(url,'',  (result) => {
                $(obj).parent('p').remove();
...
function searchQuotes(searchType) {
...
function qoutePreviewPdf(isNonClient,isPreDraftBill) {
    ...
    postData.total = total_calculation.toFixed(2);
...
function saveQuotes(qType,actionType,isSendEmail) {
